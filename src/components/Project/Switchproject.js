import * as React from 'react';
import PropTypes from 'prop-types';
import Tabs from '@mui/material/Tabs';
import Tab from '@mui/material/Tab';
import Box from '@mui/material/Box';
import Displayprojecttable from "../Project/Displayprojecttable"

import Displayclienttable from "../Client/Displayclienttable"
import Projectdocumentstable from "../Project/Projectdocumentstable";
import Projecttable from '../Client/Projecttable';
import Statusreporttable from "../Project/Statusreporttable"

function CustomTabPanel(props) {
  const { children, value, index, ...other } = props;

  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`simple-tabpanel-${index}`}
      aria-labelledby={`simple-tab-${index}`}
      {...other}
    >
      {value === index && <Box sx={{ p: 3 }}>{children}</Box>}
    </div>
  );
}

CustomTabPanel.propTypes = {
  children: PropTypes.node,
  index: PropTypes.number.isRequired,
  value: PropTypes.number.isRequired,
};

function a11yProps(index) {
  return {
    id: `simple-tab-${index}`,
    'aria-controls': `simple-tabpanel-${index}`,
  };
}

export default function Switchproject() {
  const [value, setValue] = React.useState(0);

  const handleChange = (event, newValue) => {
    setValue(newValue);
  };

  return (
    <Box sx={{ width: '100%' }}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
          {/* <Tab  style={{color:"black"}} label="Clients" {...a11yProps(0)} /> */}
          <Tab  style={{color:"black"}} label="Projects Documents" {...a11yProps(0)} />
          <Tab  style={{color:"black"}} label="Status Report" {...a11yProps(1)} />
          {/* <Tab  style={{color:"black"}} label="Client Spocs" {...a11yProps(3)} /> */}
        </Tabs>
      </Box>
      <CustomTabPanel value={value} index={0}>

      <Projectdocumentstable/>
      </CustomTabPanel>
      <CustomTabPanel value={value} index={1}>
   <Statusreporttable/>
      
      </CustomTabPanel>
      
    </Box>
  );
}
